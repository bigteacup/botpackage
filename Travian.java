...
//				maDate.toString().split("")[0];
	...
	private void cadeaux() {
		...
		try{
			...
			quete1Active.click();
			randomsleep.court();
			//WebElement  lien = compte.getDriver().findElement(By.className("button-content")).getText().contains("RÃ©compense");//*[@id="button5533e89cab960"]/div/div[2]
		}catch (Exception e) {
	...
	private void npcNegatif() {
		...
		randomsleep.court();
		compte.getDriver().findElement(By.xpath("//button[contains(@value, 'Ã‰change de ressources')]")).click();
		randomsleep.court();
	...
	private void prendrePhoto(String nom) {
		...
		{
			// Afficher le nom de chaque Ã©lÃ©ment
